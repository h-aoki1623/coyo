...
@pytest.fixture()
def service(mock_settings: MagicMock, mock_openai_client: AsyncMock) -> EmbeddingService:
    with patch("coyo.services.embedding.get_settings", return_value=mock_settings), \
...
class TestEmbed:
    ...
    async def test_returns_embeddings_in_correct_order(
        self, service: EmbeddingService, mock_openai_client: AsyncMock,
    ) -> None:
        emb1 = [0.1, 0.2, 0.3]
        emb2 = [0.4, 0.5, 0.6]
        item1 = MagicMock()
